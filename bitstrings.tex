...
\section{Operations on Bit Strings}
...
\begin{description}
	\item[$\rank_{\ph}(S, i) \rightarrow j$]
	Returns the number $j$ of $\ph$ symbols in the bit string $S$ on positions $[0, i]$.
	For convenience we extend the definition or \rank{} as $0$ for $i < 0$ and $\rank_{\ph}(S, N - 1)$ for $i \ge N$.

	\item[$\select_{\ph}(S, j) \rightarrow i$]
	Returns the position $i$ of the $j$-th symbol $\ph$ in the bit string $S$.
	If $j \le 0$, it returns $-1$; if $j > \rank_{\ph}(S, N - 1)$, it returns $N$.
\end{description}

Using these two operations we are able to derive more operations related to a given position:
\begin{description}
	\item[$S[i\char93 \rightarrow \{0, 1\}$]
...
For a given parameter $j$ we find in which block it is contained by division by $B$ and note the reminder as a block-local index.
...
If the block is dense, we find in which small block the $j$-th symbol \ph{} lays by division by $b$ and note the small-block-local index.
...
\begin{algorithmic}
\Function{\select}{$S, j$}
	\State $a \gets \frac{j}{B}; j' \gets j \% B$
	\If{$a\ \textrm{is sparse}$}
		\State \Return{$\glob[a] + \blocks[a].\blockEnum[j']$}
	\Else
		\State{$a' \gets \frac{j'}{b}; j'' \gets j' \% b$}
		\State $i \gets \glob[a] + \blocks[a].\block$
		\If{$a'\ \textrm{is sparse}$}
			\State \Return{$i + \blocks[a].\smallBlocks[a'].\smallBlockEnum[j'']$}
		\Else
			\While{$\true$}
				\State $i' \gets \select[S[i:i + s], j'']$
				\If{$i' = -1$}
					\State $j'' \gets j'' - \rank[S[i:i + s], s]$
					\State $i \gets i + s$
				\Else
					\State \Return{$i + i'$}
				\EndIf
...
Because of the additional structure, which stems from the balanced property, we define more operations on balances bit strings:
\begin{description}
	...
	The result does not have to exists in a general balanced bit string, however it will not be an issue in our case.
...
\subsection{Structure for \match{} and \enclose{}}
...
To address this problem, we use an \emph{indexed dictionary}, which we introduce in the next section.
It supports the required operations while having the space complexity (in this case) $O(\frac{N}{\log N}\log \log N) + o(\frac{N}{\log N}) + O(\log \log N) = o(N)$.
...
\begin{itemize}
	...
	\item $O(\frac{N}{\log N}\log \log N)$ -- the indexed dictionary $P'$ on the first level;
	\item $O(\frac{N}{\log^2 N}\log \log N)$ -- the indexed dictionary $P'_2$ on the second level;
...
Similarly we define Range Maximum Query $\RMQi(G, i_1, i_2)$ returning the left-most position of the maximum value in range $G[i_1 : i_2]$.
...
\begin{proof}
	...
	All the values depend only on the block $S[i : j]$ and the value of $E[i - 1]$.
