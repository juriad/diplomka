...
\Function{\lrmSearch}{$i, v$}
	...
	\State $j \gets L[l, M[\endd] - v]$ \Comment{If the index is out of the range, $-1$ is returned}
	\State \Return{$j$}
\EndFunction
\end{algorithmic}
...
\begin{algorithmic}
\Function{\rmqSize}{$S, g, i_1, i_2$}
	\State $\ms_1 \gets 0; \ms_2 \gets 0; \ms_3 \gets 0$
	\State $m \gets \rmq(S, g, i_1, i_2)$
	
	\If{$e[B(i_1) - 1] + \rmqBlock(S_{B(i_1)}, g, \prefix(i_1, i_2)) = m$} \Comment{In prefix?}
		\State $\ms_1 \gets \rmqSizeBlock(S_{B(i_1)}, g, \prefix(i_1, i_2))$
	\EndIf
	
	\If{$e[B(i_2) - 1] + \rmqBlock(S_{B(i_2)}, g, \suffix(i_1, i_2)) = m$} \Comment{In suffix?}
		\State $\ms_2 \gets \rmqSizeBlock(S_{B(i_2)}, g, \suffix(i_1, i_2))$
	\EndIf

	\State $(f, l) \gets \rmqiSpan(m, \spann(i_1, i_2))$
	\If{$m[f] = m$} \Comment{In span?}
		\State $k \gets \rank_1(\mr, m)$ \Comment{$k$-th smallest minimum}
		\State $x \gets \runFirst(\mpp, k, \rank_1(\mi_k, f))$ \Comment{Elements before the run of block $f$}
...
\begin{algorithmic}
\Function{\rmqSelect}{$S, g, i_1, i_2, r$}
	\State $m \gets \rmq(S, g, i_1, i_2)$
	\If{$e[B(i_1) - 1] + \rmqBlock(S_{B(i_1)}, g, \prefix(i_1, i_2)) = m$} \Comment{In prefix?}
		\State $\ms_1 \gets \rmqSizeBlock(S_{B(i_1)}, g, \prefix(i_1, i_2))$ \Comment{Prefix size}
		\If{$\ms_1 \ge r$}
			\State \Return{$B(i_1) B + \rmqSelectBlock(S_{B(i_1)}, g, \prefix(i_1, i_2), r)$}
		\Else
			\State $r \gets r - \ms_1$
		\EndIf
	\EndIf
	...
	\State $(f, l) \gets \rmqiSpan(m, \spann(i_1, i_2))$
	\If{$m[f] = m$} \Comment{In span?}
		\State $\ms_3 \gets \rmqSize(S, g, \spann(i_1, i_2))$
		\If{$\ms_3 \ge r$}
			\State $k \gets \rank_1(\mr, m)$ \Comment{$k$-th smallest minimum}
	...
	\If{$e[B(i_2) - 1] + \rmqBlock(S_{B(i_2)}, g, \suffix(i_1, i_2)) = m$} \Comment{In suffix?}
		\State $\ms_2 \gets \rmqSizeBlock(S_{B(i_2)}, g, \suffix(i_1, i_2))$ \Comment{Prefix size}
		\If{$\ms_2 \ge r$}
			\State \Return{$B(i_2) B + \rmqSelectBlock(S_{B(i_2)}, g, \suffix(i_1, i_2), r)$}
		\Else
