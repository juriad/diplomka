...
\Function{\decomposeSubtree}{$v, B$}
	...
	\Else
		...
		\ForAll{$u \gets \children(v)$}
			\State $(p, t, r') = \decomposeSubtree(u, B)$ \Comment{Process child}
...
\Function{\decompose}{$v, B$}
	\State $(P, t, r) \gets \decomposeSubtree(v, B)$ \Comment{Decompose the subtree of root}
	\If{$|t| \ne 0$} \Comment{A temporary component exists}
		\If{$r = -1$}
			\State $\append(P, t)$ \Comment{Declare permanent}
		\Else \Comment{There is a split vertex}
			\State $s \gets [\,]$
			\ForAll{$u \gets t$} \Comment{Revise the component}
				\State $\append(s, u)$
				\If{$u = r$} \Comment{Split vertex found}
...
\Function{\subtreeSize}{$i$}
	\If{$i.\tau_3 \ne 0$}
		...
		\If{$\isAncestor[i.\size, i.\rep, i.\tau_3, i.\dummyII]$}
			\State \Return{$s - 1 + \subtreeSize(\bottomII(i))$}
		\Else
			\State \Return{$s$}
		\EndIf
	\ElsIf{$i.\tau_2 \ne 0$}
...
The algorithm for the operation \dfudsRank{} consists mostly of the computation of \dfudsAnc{}.
...
We associate each node $n$ with the depth of the vertex which it represents in the original tree.
...
The TC data structure supports all operations of FF; it is better in ranking operations (\postRank{}, \postSelect{}, \dfudsRank{}, \dfudsSelect{}) which are not supported by the former one.
...
The data structure provides the following three operations in running time $O(1)$:
\begin{description}
