...
We follow on \ref{ss:rmq-def} and define operations on the array $G$.
\begin{itemize}
...
\begin{algorithmic}
	\Function{rmq\_rank}{$G, i, j, k$}
		\If{$rmq(G, i, j) \ne rmq(G, i, k)$}
			\State \Return{$0$}
		\Else
			\State \Return{$rmq\_size(G, i, k)$}
		\EndIf
	\EndFunction
\end{algorithmic}
	...
	\listpart{We also define two more arrays in addition to $E$:}
...
\begin{algorithmic}
\Function{fwd\_search}{$G, i, d$}
	\State $p \gets table_1[S_{b(i)}, i \% b, d]$ \Comment{Position of $d$ in the small block $b(i)$}
	\If{$p \ne -1$}
		\State \Return{$p + b(i) b$}
	\Else
		\State $v \gets G[i] + d$
		\State $n \gets b(i)$ \Comment{The current node, initialized to be a leaf}
		\While{$(j \gets table_2^*[right\_siblings(n), v]) = -1$} \Comment{Sibling covering $v$}
		...
		\While{$\boolnot is\_leaf(j)$}
			\State{$j \gets table_2^*[children(j), v]$} \Comment{The first child covering $v$}
		\EndWhile
		\State $d' \gets v - e[j - 1]$
		\State \Return{$b(j) b + table_3[S_{b(j)}, d']$} \Comment{The first occurrence of $d'$}
	\EndIf
\EndFunction
\end{algorithmic}
...
\begin{algorithmic}
\Function{rmq\_list}{$G, i, j$} \Comment{$b(i) \ne b(j)$}
...
\Function{rmq}{$G, i, j$}
	\If{$b(i) = b(j)$}
		\State \Return{$table_1[S_{b(i)}, i \% b, j \% b]$}
	\Else
...
\begin{algorithmic}
\Function{rmq\_select}{$G, i, j, n$}
	\If{$b(i) = b(j)$}
		\State \Return{$table_1[S_{b(i)}, i \% b, j \% b, n]$}
	\Else
		\State $list \gets rmq\_list(G, i, j)$
		\State $m \gets rmq(G, i, j)$
		\State $ms \gets 0$
