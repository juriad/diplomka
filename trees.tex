...
\Function{is\_ancestor}{$i, j$}
	\If{$is\_leaf(i)$}
		\State \Return{$i = j$}
	\Else
		\State $end \gets find\_close(enclose(i))$ \Comment{Last one in unmatched in the subtree}
		\State \Return{$j \ge i \booland j \le end$}
	\EndIf
\EndFunction
\end{algorithmic}
...
\Function{subtree\_size}{$i$}
	...
	\Else
		\State $end \gets find\_close(enclose(i))$ \Comment{Last one in unmatched in the subtree}
		\State \Return{$rank_)(end) - rank_)(i)$}
	\EndIf
\EndFunction
\end{algorithmic}
